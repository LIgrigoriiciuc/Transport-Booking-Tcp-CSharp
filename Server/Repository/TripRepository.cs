...
public class TripRepository : GenericRepository<long, Trip>
{
    ...
    protected override string BuildInsertSql() =>
        "INSERT INTO trips (destination, time, busNumber) VALUES (@destination, @time, @busNumber)";
    ...
    protected override void SetInsertParameters(SqliteCommand command, Trip trip)
    {
        ...
        command.Parameters.AddWithValue("@time", DateTimeUtils.FormatDateTime(trip.Time));
        command.Parameters.AddWithValue("@busNumber", trip.BusNumber);
    }
 
    protected override string BuildUpdateSql() =>
        "UPDATE trips SET destination = @destination, time = @time, busNumber = @busNumber WHERE id = @id";
    ...
    protected override Trip MapResultSetToEntity(SqliteDataReader reader)
    {
        ...
        DateTime time = DateTimeUtils.Parse(reader.GetString(reader.GetOrdinal("time")));
        string busNumber = reader.GetString(reader.GetOrdinal("busNumber"));
        return new Trip(id, destination, time, busNumber);
    }
}